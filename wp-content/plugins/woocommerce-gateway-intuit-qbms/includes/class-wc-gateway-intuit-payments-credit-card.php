...
class WC_Gateway_Inuit_Payments_Credit_Card extends WC_Gateway_Inuit_Payments {
	...
	protected function validate_credit_card_fields( $is_valid ) {
		...
		// card type
		if ( ! in_array( Framework\SV_WC_Helper::get_post( 'wc-' . $this->get_id_dasherized() . '-card-type' ), $valid_card_types, true ) ) {
	...
	public function get_order( $order_id ) {
		...
		if ( isset( $order->payment->js_token ) ) {
			...
			// expiry month/year
			list( $order->payment->exp_month, $order->payment->exp_year ) = array_map( 'trim', explode( '/', Framework\SV_WC_Helper::get_post( 'wc-' . $this->get_id_dasherized() . '-expiry' ) ) );

			// card data
			$order->payment->card_type = Framework\SV_WC_Helper::get_post( 'wc-' . $this->get_id_dasherized() . '-card-type' );
		}
		...
		if ( $this->get_plugin()->is_subscriptions_active() ) {
			...
			$order->payment->is_recurring = Framework\SV_WC_Plugin_Compatibility::is_wc_subscriptions_version_gte_2_0() ? wcs_order_contains_renewal( $order_id ) : WC_Subscriptions_Order::order_contains_subscription( $order_id );
		}
