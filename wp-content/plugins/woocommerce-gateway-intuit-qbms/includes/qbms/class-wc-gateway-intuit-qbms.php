...
class WC_Gateway_Intuit_QBMS extends Framework\SV_WC_Payment_Gateway_Direct {
	...
	public function render_view_transaction_form() {
		...
		if ( ! $order || $this->get_id() !== Framework\SV_WC_Order_Compatibility::get_prop( $order, 'payment_method' ) ) {
			return;
		}

		// search form parameters
		$transaction_id = $this->get_order_meta( $order, 'trans_id', true );
		$date           = ( Framework\SV_WC_Order_Compatibility::get_date_paid( $order ) ) ? Framework\SV_WC_Order_Compatibility::get_date_paid( $order )->date( 'm/d/Y' ) : '';
