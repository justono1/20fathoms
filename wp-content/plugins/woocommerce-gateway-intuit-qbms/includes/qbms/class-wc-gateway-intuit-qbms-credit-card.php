...
class WC_Gateway_Intuit_QBMS_Credit_Card extends WC_Gateway_Intuit_QBMS {
	...
	public function get_order( $order_id ) {
		...
		if ( $this->is_environment( 'test' ) && Framework\SV_WC_Helper::get_post( 'wc-intuit-qbms-credit-card-test-condition' ) ) {
			$order->payment->test_condition = Framework\SV_WC_Helper::get_post( 'wc-intuit-qbms-credit-card-test-condition' );
		}
	...
	public function get_order_for_capture( $order, $amount = null ) {
		...
		$order->capture->request_id = Framework\SV_WC_Order_Compatibility::get_prop( $order, 'id' ) . '-' . rand();
	 ...
	 *
	 * @see Framework\SV_WC_Payment_Gateway_Direct::do_credit_card_capture_failed()
	 *
	 * @since 2.3.1
	 *
	 * @param \WC_Order $order order object
	 * @param Framework\SV_WC_Payment_Gateway_API_Response
	 */
	protected function do_credit_card_capture_failed( \WC_Order $order, Framework\SV_WC_Payment_Gateway_API_Response $response ) {
	 ...
	 *
	 * @since 1.1
	 * @see Framework\SV_WC_Payment_Gateway_Direct::add_payment_gateway_capture_data()
	 * @param \WC_Order $order the order object
	...
	protected function add_payment_gateway_capture_data( $order, $response ) {
		...
		$this->update_order_meta( $order, 'capture_authorization_code', $response->get_authorization_code() );
		$this->update_order_meta( $order, 'capture_client_trans_id',    $response->get_client_trans_id() );
	...
	protected function get_order_for_refund( $order, $amount, $reason ) {
		...
		//  the moment since there's no real way to test.  For further info:
		//  https://developer.intuit.com/docs/030_qbms/0060_documentation/error_handling#QBMS_Error_Recovery
		$order->refund->request_id = Framework\SV_WC_Order_Compatibility::get_prop( $order, 'id' ) . '-' . rand();
