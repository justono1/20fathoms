...
class WC_Intuit_Payments_API extends Framework\SV_WC_API_Base implements Framework\SV_WC_Payment_Gateway_API {
	 ...
	 * @param Framework\SV_WC_Payment_Gateway_Payment_Token $token the token object
	 ...
	 * @param string $customer_id the customer ID
	 * @return Framework\SV_WC_API_Get_Tokenized_Payment_Methods_Response
	 * @throws Framework\SV_WC_API_Exception
	 */
	...
	protected function do_pre_parse_response_validation() {
		...
		if ( '2.0' === $this->get_oauth_version() && 401 === $this->get_response_code() ) {
			...
			// for legacy OAuth 1.0 connections
			update_option( 'wc_intuit_payments_connected', 'no' );

			throw new Framework\SV_WC_API_Exception( 'Invalid API credentials.' );
		}
	...
	protected function maybe_refresh_access_tokens() {
		...
		// handle legacy OAuth 1.0 reconnect
		if ( '1.0' === $this->get_oauth_version() ) {

			if ( $this->get_gateway()->can_reconnect() ) {

				$return = $this->maybe_reconnect_legacy();

				$this->request_method = 'POST';
			}

		} elseif ( time() >= $this->get_gateway()->get_access_token_expiry() ) {
	...
	/**
	 * Maybe reconnect to OAuth 1 if the token is within the refresh window.
	 *
	 * @since 2.1.1
	 *
	 * @return bool
	 */
	protected function maybe_reconnect_legacy() {

		if ( $this->get_gateway()->can_reconnect() ) {

			try {

				$response = $this->oauth_reconnect();
				...
				$this->get_plugin()->log( 'Could not reconnect to Intuit. ' . $e->getMessage(), $this->get_gateway()->get_id() );
